...
class PostgresWriteMixin(PostgresEmailOutboxMixin):
    async def _bump_revision(self, connection: Any) -> int:
        return int(
    ...
    async def _audit(
        self, connection: Any, actor_id: str, action: str, entity_type: str, entity_id: str
    ) -> None:
        item = {
            "id": str(uuid4()),
            "at": _now_iso(),
        ...
        await connection.execute(
            ...
            item["id"], actor_id, action, entity_type, entity_id,
            _now_datetime(), json.dumps(item, ensure_ascii=False),
        )
    ...
    async def _notification(
        ...
        await connection.execute(
            ...
            (id, target_type, target_id, kind, payload)
            VALUES ($1, 'user', $2, 'system', $3::jsonb)""",
            item["id"], user_id, json.dumps(item, ensure_ascii=False),
        )
        ...
        if user_row and user_row["email_verified"]:
            await self._enqueue_email(
                connection, user_row["email"], "notification",
                {"title": title, "message": message},
            )

    async def create_password_reset_atomic(
        self, email: str, reset: dict, email_message: dict, now_ms: int, cooldown_ms: int
    ) -> bool:
            ...
            async with connection.transaction():
                ...
                if existing:
                    ...
                    await connection.execute(
                        ...
                        reset["id"], email, int(reset["expiresAtMs"]), json.dumps(reset, ensure_ascii=False),
                    )
                else:
                    await connection.execute(
                ...
                await self._enqueue_email(connection, email, "password-reset", email_message)
                await self._bump_revision(connection)
        return True

    async def reset_password_atomic(
        self, email: str, expected_code_hash: str, password_hash: str, max_attempts: int,
    ) -> dict:
        async with self.pool.acquire() as connection:
            async with connection.transaction():
                await connection.fetchval(
                ...
                if not row:
                    raise PersistenceError("Код восстановления недействителен или истёк. Запросите новый код.", 422)
                reset = payload(row["payload"])
                now_ms = _now_ms()
                if int(reset.get("expiresAtMs", 0)) <= now_ms:
                    await connection.execute("DELETE FROM lug_password_resets WHERE id = $1", row["id"])
                    await self._bump_revision(connection)
                    raise PersistenceError("Код восстановления недействителен или истёк. Запросите новый код.", 422)
                if int(reset.get("attempts", 0)) >= max_attempts:
                    await connection.execute("DELETE FROM lug_password_resets WHERE id = $1", row["id"])
                    await self._bump_revision(connection)
                    raise PersistenceError("Лимит попыток исчерпан. Запросите новый код восстановления.", 422)
                if not compare_digest(expected_code_hash, str(reset.get("codeHash") or "")):
                    reset["attempts"] = int(reset.get("attempts", 0)) + 1
                    await connection.execute(
                        "UPDATE lug_password_resets SET payload=$2::jsonb, updated_at=now() WHERE id=$1",
                        row["id"], json.dumps(reset, ensure_ascii=False),
                    )
                    await self._bump_revision(connection)
                    raise PersistenceError("Код восстановления недействителен или истёк. Запросите новый код.", 422)
                user_row = await connection.fetchrow(
                ...
                await connection.execute(
                    ...
                    user_row["id"], json.dumps(user, ensure_ascii=False),
                )
                await connection.execute("DELETE FROM lug_password_resets WHERE id = $1", row["id"])
                await connection.execute("DELETE FROM lug_sessions WHERE user_id = $1", user_row["id"])
                await self._audit(connection, user_row["id"], "auth.password_reset", "user", user_row["id"])
                await self._bump_revision(connection)
        return user
    ...
    async def update_settings_atomic(self, patch: dict, actor_id: str) -> dict:
        async with self.pool.acquire() as connection:
            async with connection.transaction():
                ...
                await self._audit(connection, actor_id, "settings.updated", "settings", "global")
                await self._bump_revision(connection)
        return settings

    async def update_quota_atomic(self, team_id: str, confirmed: bool, actor_id: str) -> dict:
        async with self.pool.acquire() as connection:
            async with connection.transaction():
                row = await connection.fetchrow(
                ...
                await self._audit(connection, actor_id, "team.quota_updated", "team", team_id)
                await self._bump_revision(connection)
        return team
        ...
        async with self.pool.acquire() as connection:
            async with connection.transaction():
                ...
                team.setdefault("review", {})[field] = {
                    "status": status, "comment": comment, "updatedAt": _now_iso()
                }
                ...
                await self._audit(connection, actor_id, f"team.{field}.{status}", "team", team_id)
                if team.get("captainId"):
                    title = "Проверка поля пройдена" if status == "approved" else "Поле требует исправления" if status == "rejected" else "Проверка поля обновлена"
                    await self._notification(connection, team["captainId"], title, comment or "Оргкомитет обновил решение по карточке команды.")
                await self._bump_revision(connection)
        ...
        async with self.pool.acquire() as connection:
            async with connection.transaction():
                target_row = await connection.fetchrow(
                    "SELECT team_id FROM lug_users WHERE id = $1 AND role <> 'admin'", user_id
                )
                if not target_row:
                    raise PersistenceError("Пользователь не найден.", 404)
                team_id = target_row["team_id"]
                if team_id:
                    await connection.fetchrow("SELECT id FROM lug_teams WHERE id = $1 FOR UPDATE", team_id)
                target_row = await connection.fetchrow(
                    "SELECT payload FROM lug_users WHERE id = $1 FOR UPDATE", user_id
                )
                target = payload(target_row["payload"])
                target.update({"identityStatus": status, "identityComment": comment, "isIdentityConfirmed": status == "approved"})
                await connection.execute(
                    "UPDATE lug_users SET payload = $2::jsonb, updated_at = now() WHERE id = $1",
                    user_id, json.dumps(target, ensure_ascii=False),
                )
                members, settings = [], payload(await connection.fetchval("SELECT payload FROM lug_settings WHERE id = 1"))
                if team_id:
                    rows = await connection.fetch("SELECT payload FROM lug_users WHERE team_id = $1 ORDER BY updated_at", team_id)
                    members = [payload(row["payload"]) for row in rows]
                    team_row = await connection.fetchrow("SELECT payload FROM lug_teams WHERE id = $1 FOR UPDATE", team_id)
                    team = payload(team_row["payload"])
                    team["isAdmitted"] = domain.team_is_admitted({"settings": settings, "users": members}, team, members)
                    await connection.execute("UPDATE lug_teams SET payload = $2::jsonb, updated_at = now() WHERE id = $1", team_id, json.dumps(team, ensure_ascii=False))
                await self._audit(connection, actor_id, f"identity.{status}", "user", user_id)
                title = "Личность подтверждена" if status == "approved" else "Нужно уточнить данные" if status == "rejected" else "Проверка личности обновлена"
                await self._notification(connection, user_id, title, comment or "Документы проверены организаторами.")
                await self._bump_revision(connection)
