...
export function createWebServer({
  config,
  logger = createLogger("lug-web"),
  metrics = createMetrics("lug-web"),
}) {
  const server = createServer((req, res) => {
    ...
    res.on("finish", () => {
      metrics.increment(`http_requests.${res.statusCode}`);
      metrics.observe("http_request_duration", Date.now() - startedAt);
    ...
    try {
      if (!allowedHost(req.headers.host, config.allowedHosts))
        return rejectRequest(req, res, "Недопустимый Host.");
      const url = new URL(
        req.url || "/",
      ...
      const proxyIsTrusted =
        config.trustProxy &&
        config.trustedProxyIps.has(normalizeAddress(clientAddress(req)));
      if (
        config.requireHttps &&
        !isSecureRequest(req, proxyIsTrusted) &&
      ...
      if (url.pathname === "/healthz") {
        ...
        res.end(JSON.stringify({ status: "ok", service: "lug-web" }));
        return;
      }
      if (
        url.pathname === "/readyz" ||
...
function ensureCsrfCookie(req, res, config) {
  if (!parseCookies(req.headers.cookie || "")[CSRF_COOKIE])
    res.__csrfCookie = serializeCsrfCookie(
...
function proxy(req, res, { config, id, logger, operations = false }) {
  ...
  if (
    ...
    declaredLength > config.maxProxyBodyBytes
  ) {
  ...
  const proxyIsTrusted =
    config.trustProxy &&
    config.trustedProxyIps.has(normalizeAddress(clientAddress(req)));
  const forwardedFor = proxyIsTrusted
  ...
  const headers = {
    ...
    "x-forwarded-for": [forwardedFor, clientAddress(req)]
      .filter(Boolean)
      .join(", "),
    "x-forwarded-proto": proxyIsTrusted
      ? String(
  ...
  if (operations)
    headers.authorization = config.operationsToken
  ...
  const upstream = httpRequest(
    ...
    (upstreamResponse) => {
      ...
      if (!cookies[CSRF_COOKIE] && !res.getHeader("Set-Cookie"))
        res.setHeader(
          "Set-Cookie",
          serializeCsrfCookie(
            randomBytes(24).toString("hex"),
  ...
  upstream.on("error", (error) => {
    ...
    logger.error("proxy.failed", {
      requestId: id,
      traceId: res.__traceParent.split("-")[1],
      error,
    });
    if (!res.headersSent) {
      res.writeHead(502, {
      ...
      res.end(JSON.stringify({ error: "Сервис API временно недоступен." }));
...
function normalizeTraceparent(value) {
  ...
  if (
    /^00-[0-9a-f]{32}-[0-9a-f]{16}-[0-9a-f]{2}$/.test(candidate) &&
    !/^00-0{32}-0{16}-/.test(candidate)
  )
    return candidate;
...
function rejectPayload(req, res, maxBytes) {
  ...
  res.end(
    JSON.stringify({
      error: `Файл или запрос превышает лимит ${Math.round(maxBytes / 1024 / 1024)} МБ.`,
    }),
  );
}
...
export function startWebServer(options) {
  ...
  return new Promise((resolve, reject) => {
    ...
    server.listen(options.config.webPort, options.config.webHost, () =>
      resolve(server),
    );
  });
}
